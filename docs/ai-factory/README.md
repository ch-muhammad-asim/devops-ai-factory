# AI Factory: concepts and guides
...
This README is the **start-here guide for the AI Factory documentation**. It combines the beginner vocabulary, mental models, comparisons, and links to the deeper implementation guides in this directory.

The repository-level [README](../../README.md) remains the higher-level AI Factory architecture and platform overview.
...
### Temperature vs reasoning / thinking effort

These are **different controls** and should not be described as the same thing.

| Concept | Temperature | Reasoning / thinking effort |
|---|---|---|
| What it changes | How the model samples/selects output tokens | How much computation or deliberation a supported reasoning model uses |
| Simple meaning | **How adventurous should the wording/output be?** | **How much work should the model spend solving the problem?** |
| Low setting | More predictable, conservative output | Less deliberation; usually faster |
| High setting | More varied or creative output | More deliberation; usually slower and potentially more expensive |
| Makes the model smarter? | **No** | It can help on difficult tasks, but does not guarantee correctness |
| Mainly affects | Output variation / randomness | Problem-solving compute / reasoning depth |
| Good example | Brainstorming names vs returning a stable command | Easy definition vs difficult architecture/debugging problem |

A simple human analogy:

~~~text
Reasoning effort
= how long and carefully you think before answering.

Temperature
= how adventurous you are when choosing the wording
...
~~~text
temperature     = output randomness / variation
reasoning effort = problem-solving effort / compute
~~~

Also remember that **reasoning effort is not a universal LLM parameter**. Some models or APIs expose a reasoning/thinking control and others do not.
...
## Key comparisons - concepts people often mix up

The tables below are designed as a quick reference for juniors and non-technical readers.

### Temperature vs reasoning effort

| Temperature | Reasoning effort |
|---|---|
| Controls sampling/output variation | Controls deliberation/computation where supported |
| Higher can make wording more diverse | Higher can spend more work on difficult problems |
| Does not mean "think harder" | Does not mean "be more creative" |
| Useful for controlling style/variation | Useful for complex coding, math, planning, or analysis |

### Prompting vs RAG vs fine-tuning vs tools

| Technique | What changes? | Best use |
|---|---|---|
| Prompt engineering | Instructions/context sent in the request | Tell the model how to perform the task |
| RAG | Adds retrieved information to the request | Give the model current/private documents |
| Fine-tuning | Changes model weights through training | Teach persistent specialized behavior |
| Tool calling | Connects the application to live systems/functions | Get live data or perform actions |

Example:

~~~text
Need the model to answer in a table?
-> Prompting

Need today's internal runbook?
-> RAG

Need live Kubernetes pod status?
-> Tool/API call

Need specialized behavior that prompting cannot reliably provide?
-> Fine-tuning may be considered
~~~

### Model vs inference runtime vs gateway vs agent

| Layer | Example | Job |
|---|---|---|
| Model | Gemma, Llama, Qwen | Learned intelligence/weights |
| Inference runtime | llama.cpp, vLLM | Loads and executes the model |
| LLM gateway | LiteLLM | Routes, authenticates, limits, and observes model API traffic |
| Agent | Incident investigation agent | Uses a model plus tools/state/loop to achieve a goal |
...
| Context window | Application memory |
|---|---|
| What the model can see in the current request/session context | Information saved outside the model for later use |
| Limited by token capacity | Limited by the application's storage/design |
| Disappears when no longer supplied | Can persist in databases, profiles, summaries, vector stores |
| Does not retrain the model | Does not normally change model weights |

### Training vs inference

| Training | Inference |
|---|---|
| Learns/updates model weights | Uses existing weights |
| Expensive and compute intensive | Usually much cheaper than training |
| Produces or adapts a model | Produces answers/predictions |
| Example: fine-tuning Gemma | Example: asking Gemma a question through llama.cpp |

### RAG vs fine-tuning

| RAG | Fine-tuning |
|---|---|
| Adds knowledge at request time | Changes weights |
| Good for frequently changing/private information | Good for persistent behavior/task adaptation |
| Easy to update documents | Requires another training/adaptation cycle |
| Can cite retrieved sources | Does not automatically provide source citations |

A practical rule:

> **Knowledge changes often -> think RAG. Behavior needs changing -> consider fine-tuning.**
...
| RAG | Vector database |
|---|---|
| End-to-end retrieval + generation pattern | One possible storage/search technology |
| Includes retrieval, context construction, and LLM generation | Stores/searches vectors |
| Can use vector search, keyword search, SQL, APIs, or combinations | Does not itself generate the final LLM answer |

A vector database is **optional infrastructure inside some RAG designs**, not the definition of RAG.

### Agent vs deterministic workflow

| Agent | Deterministic workflow |
|---|---|
| Model chooses the next action dynamically | Steps are predefined |
| Flexible but harder to control/test | Predictable and easier to audit |
| Useful when the path cannot be fully known in advance | Best when the process is known |
| Needs stronger guardrails/tool permissions | Usually simpler and safer |

Use a normal workflow when a normal workflow is enough.

### MCP vs API/tool calling

| MCP | Direct API/tool integration |
|---|---|
| Standard protocol for exposing/discovering tools and context | Custom/direct integration with a specific API or function |
| Useful when many AI clients/tools need a common interface | Often simplest for one application and one integration |
| Provides a standardized ecosystem boundary | Gives maximum application-specific control |

MCP does not replace HTTP, databases, Kubernetes APIs, or business APIs. It provides a standard AI-facing way to expose capabilities.

### Parameters vs generation parameters

| Model parameters | Generation parameters |
|---|---|
| Learned weights inside the model | Runtime settings used while generating |
| Billions of parameters may describe model size/capacity | Temperature, top-p, top-k, max tokens, etc. |
| Learned during training | Usually chosen at inference time |
| Changing them requires training/adaptation | Changing them does not retrain the model |

### Quantization vs smaller model

| Quantized model | Smaller model |
|---|---|
| Same/similar architecture represented at lower numerical precision | Model architecture has fewer parameters |
| Reduces storage/memory requirements | Naturally requires fewer resources |
| May trade some quality for efficiency | May have lower capability depending on task |
| Example: Q4 version of a larger model | Example: choosing a 2B model instead of a 12B model |

### GPU vs inference runtime

| GPU | Inference runtime |
|---|---|
| Hardware accelerator | Software |
| Performs parallel computation | Loads model, manages memory, batching, API serving |
| Example: NVIDIA L4 or Apple GPU | Example: llama.cpp or vLLM |

The GPU does not know how to serve an HTTP chat API by itself. The runtime uses the GPU.
...
## Technical implementation guides in this directory

After the concepts above, continue with the implementation-focused documents:

- [Local Apple-Silicon AI playground](ai-playground/README.md) - run a GPU-scheduled model pod locally with minikube + krunkit.
- [Inference workloads](inference-workloads.md) - inference, GPU integration, vLLM, KServe, and KubeRay.
- [LLM gateway](llm-gateway/README.md) - LiteLLM and model-access architecture.
- [LLM Gateway with Gateway API](llm-gateway/gateway-api.md) - Traefik Gateway API path to LiteLLM.
...
- [Architecture SVG](architecture.svg) / [PNG](architecture.png) - simplified AI Factory architecture.
- [Kubernetes platform guide](../platform/README.md) - the Kubernetes substrate these AI services run on.

Use this README as the vocabulary and mental-model layer, then move into the implementation guides only when the underlying concept is clear.
